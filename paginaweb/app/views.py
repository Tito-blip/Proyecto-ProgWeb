...
def mensajes():
    if current_user.is_emp or current_user.is_admin:
        base = db.session.query(Contact).all() # Consigue los mensajes desde la db para pasarlos a la plantilla.
...
@login_required
def redirect_update(): # Recibe el id desde el formulario y redirecciona a la pagina de update con el id como parametro
...
def update(id):
    cliente = Clientes.query.get(id) # Utilizando el id del redirect anterior consigue el usuario desde la bd

    if not cliente:
        return abort(404)

    if request.method == "POST" and cliente and (current_user.is_emp or current_user.is_admin):        
            cliente.rut = request.form['rut'] # Recibe los campos del formulario para la base de datos directamente
            ...
            db.session.commit() # Guarda los cambios y envia mensaje de confirmación
...
# Tabla Consultas Update (Solo demostración) #

@views_pagina.route('/tableConsultas_redirect')
@login_required
def redirect_consulta():
    id = request.args.get("id")

    if id and (current_user.is_emp or current_user.is_admin):
        return redirect(url_for('views_pagina.consultas', id=id))
    abort(401)

@views_pagina.route("/tableConsulta/<int:id>", methods=['GET', 'POST']) # Actualizar, con metodo GET para mostrar datos anteriores
@login_required

def consultas(id):
    consulta = Contact.query.get(id)

    if not consulta:
        return abort(404)
    # Cambia el estado de la consulta para ocultar boton de responder
    if request.method == "POST" and (current_user.is_emp or current_user.is_admin):        
            consulta.status = 'Respondido'

            db.session.commit()
            flash(f"Respuesta enviada a {consulta.email}", category='success') # Devuelve mensaje de exito con email
            return redirect('/mensajes')             
    return render_template("forms/consultaUpdate.html", consulta = consulta, id = id)

# Errores #

@views_pagina.app_errorhandler(401) # Unauthorized
def unauthorized(e):
    return render_template('errors/401.html'), 401

@views_pagina.app_errorhandler(404) # Not Found
...
@views_pagina.app_errorhandler(500) # Server Error
def error_server(e):
